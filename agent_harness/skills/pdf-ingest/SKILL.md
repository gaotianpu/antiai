---
name: pdf-ingest
description: PDF 下载→转换→格式清理，产出格式完好的 Markdown 文档。提示词：pdf 摄入, pdf转md, 论文格式化
compatibility: Requires markitdown (pip install markitdown[pdf]), curl, and writable RAW_DIR/pdf/ directory.
metadata:
  variables:
...
> markitdown 将公式渲染为纯文本（非 LaTeX），如需 LaTeX 公式可尝试 `pandoc --from pdf --to markdown`。
