...
### 1.1 多头自注意力
![self-attention](../../raw/images/Transformer/fig_2.png) <br/>
图3：自注意力机制和多头自注意力机制。
...
![Figure 11](../../raw/images/vit/fig_11.png)<br/>
...
## 2. 基础的Block
![The Transformer model architecture](../../raw/images/Transformer/fig_1.png)<br/>
...
![MAGNETO](../../raw/images/MAGNETO/fig_1.png)<br/>
图8：MAGNETO Block示意图。  

LayerNorm的使用: [原始论文](../../raw/nlp/transformer.md)、[BERT](../../raw/nlp/bert.md)、NMT/[BART](../../raw/nlp/BART.md)中LN是在多头注意力，FFN的后面, 即Post-LN; 而[GPT](../../raw/nlp/gpt.md),[ViT](../../raw/vit/ViT.md)等是在多头注意力，FFN的前面，即Pre-LN。

而在[MAGNETO](../../raw/Multimodal/MAGNETO.md)中，多头注意力后加入了一层LN，FFN中激活函数后加了一层LN。这种方式在适应多模输入、训练时的稳定性，以及实际效果方面都比较有优势。
...
残差概念最早是在[Resnet](../../raw/cnn/resnet.md)中提出，主要目的是给梯度传播提供一条额外的路径，在深层模型结构中，有助于更好的梯度传播，减少梯度消失的情况，加速收敛，提升训练时期的稳定性。
...
当训练和推理时的长度一致时，二者差别并不大，但当推理时的输入长度大于训练时的输入长度时，sin/cos可以通过两个相邻位置间插值的方式实现外推(推理长度大于训练长度)。关于推理长度和训练长度不一致后续还有更多的探索,可以参考以下几篇论文：[XPos](../../raw/nlp/XPOS.md), [ALibi](../../raw/nlp/Alibi.md), [RoPE](../../raw/nlp/RoFormer.md)。主要思路：1. 位置嵌入不只是放在初始位置，而是放在每个self-attention中的Q,K上；2.位置嵌入和token嵌入的作用方式，分别采用加法、乘法、或者乘法+加法 3.用其他位置-维度值的算法优化sin/cos方式。


## 4. 整体架构
### 4.1 编码器 + 解码器 架构
原始论文[Attention Is All You Need](../../raw/nlp/transformer.md)中面向的是一个翻译任务，需要将一种语言翻译成另外一种语言，使用到到的架构模型时，编码器-解码器 结构（见上图6）。 结合上一节自注意力模块，编码器采用双向编码机制，即Query需要查询计算与上下文中所有的语义单元的关系。而解码器则是单向编码机制，即query只查询过去的，在它之前打的语义单元的关系。
...
### 4.2 双向注意力编码
代表：[BERT](../../raw/nlp/bert.md)

![BERT的预训练和微调过程](../../raw/images/bert/fig_1.png)<br/>
...
### 4.3 单向(因果)注意力编码
代表：[GPT-1](../../raw/nlp/gpt.md)

![GPT的预训练和微调过程](../../raw/images/gpt_1/fig_1.png)<br/>
...
## 参考
1. [Attention Is All You Need](../../raw/nlp/transformer.md) 介绍Transfomer的原始论文
2. [MAGNETO:Foundation Transformers](../../raw/Multimodal/MAGNETO.md) LayerNorm和初始化方面的优化
3. 推理长度可外推：[XPos](../../raw/nlp/XPOS.md), [ALibi](../../raw/nlp/Alibi.md), [RoPE](../../raw/nlp/RoFormer.md)
4. [Resnet：Deep Residual Learning for Image Recognition](../../raw/cnn/resnet.md) 残差连接
