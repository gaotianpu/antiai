...
由于互联网上的分割掩码并不丰富，我们建立了一个数据引擎来收集我们的1.1亿掩码数据集SA-1B。数据引擎有三个阶段：（1）模型辅助的手工标注阶段，（2）混合了自动预测掩码和模型辅助标注的半自动阶段，以及（3）全自动阶段，在该阶段中，我们的模型在没有标注人员输入的情况下生成掩码。我们将详细介绍下一步。 <!--主动学习？-->
