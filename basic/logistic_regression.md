...
图 1：二分类问题示例。 在一个2D空间(平面)中，分布着2类点集,这些点看成2维向量，也有相应的标签对应，正样本，label=1(绿色)，负样本,label=0(蓝色)。 需要找到一个分割线(y=ax+b)将空间分成2个部分(3D空间中是找到一个分割平面，3D+空间中，是找到一个超平面，将空间分成2个部分)，将这两类点分割开来。
