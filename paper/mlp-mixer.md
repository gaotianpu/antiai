...
尽管它很简单，但Mixer还是取得了具有竞争力的结果。当在大数据集(即，∼1亿张图像)，在精度/成本权衡方面，它达到了CNN和Transformers之前宣称的接近最先进的性能。这包括ILSVRC2012“ImageNet”上87.94%的顶级验证精度[13]。当对更适度规模的数据进行预训练时(即，∼1–10M图像)，再加上现代的正则化技术[49，54]，Mixer也实现了强大的性能。然而，与ViT类似，它略低于专门的CNN架构。
