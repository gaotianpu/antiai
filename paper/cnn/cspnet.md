...
ImageNet。在ImageNet图像分类实验中，所有超参数，如训练步骤、学习速率计划、优化器、数据增广等，我们都遵循Redmonet al [29]中定义的设置。对于基于ResNet的模型和基于ResNeXt的模型，我们设置了8000000个培训步骤。对于基于DenseNet的模型，我们设置了1600000个培训步骤。我们将初始学习速率设置为0.1，并采用多项式衰减学习速率调度策略。动量和权重衰减分别设置为0.9和0.005。所有架构都使用单个GPU进行批量大小为128的通用训练。最后，我们使用ILSVRC 2012的验证集来验证我们的方法。
...
实验结果表明，所提出的EFM比GFM慢2 fps，但其AP和AP50分别显著提高了2.1%和2.4%。尽管引入GIoU可以将AP升级0.7%，但AP50却显著降低了2.7%。然而，对于边缘计算，真正重要的是对象的数量和位置，而不是它们的坐标。因此，我们不会在后续模型中使用GIoU培训。SAM使用的注意机制与SPP增加FoV机制相比可以获得更好的帧速率和AP，因此我们使用EFM(SAM)作为最终架构。此外，虽然具有swish激活的CSPPelleeNet可以将AP提高1%，但其操作需要在硬件设计上查找表来加速，我们最终也放弃了swish激活函数。
