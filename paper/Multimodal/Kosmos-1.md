...
对于视觉问答任务，我们分别在 VQAv2 [GKSS+17] 的测试开发集和 VizWiz [GLS+18] 的测试开发集上评估零样本和少样本结果。 图片的分辨率为224×224。 我们使用贪心搜索进行解码。 在计算 VQA 精度时，我们遵循 VQAv2 评估代码的归一化规则 (5 https://github.com/GT-Vision-Lab/VQA)。 我们在 KOSMOS-1 生成答案并在 \</s>(“序列结尾”)令牌处停止的开放式设置中评估 VQA 的性能。 对于视觉问答任务，提示是“问题：{问题}答案：{答案}”。
